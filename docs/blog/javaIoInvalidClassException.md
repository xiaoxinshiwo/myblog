...
```
根据异常提示和晓波写的序列化之异常我们轻易的就可以知道这是个序列化异常，本地class的serialVersionUID和传输过来的不一致导致的。
